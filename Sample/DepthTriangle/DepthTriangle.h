...
class DepthTriangle : public Application
{
private:
    constexpr static std::size_t DefaultResourceSize = 3;

    vkpp::CommandPool mCommandPool;
    vkpp::Image mDepthImage;
    vkpp::ImageView mDepthImageView;
    vkpp::DeviceMemory mDepthImageMemory;
...
public:
    DepthTriangle(const char* apApplicationName, uint32_t aApplicationVersion, const char* apEngineName = nullptr, uint32_t aEngineVersion = 0);
    virtual ~DepthTriangle(void);
};
